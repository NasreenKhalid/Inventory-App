import React, { Component } from "react";
import Form from "./form";

//component that renders individual menu item stored in redux store
export default class MenuItem extends Component {
  constructor(props) {
    super(props);
    this.state = {
      name: "",
      price: null,
      qty: null,
      qtysold: null,
      pricesold: null,
      dateadded: null,
      openEditForm: false
    };
  }

  //setting necessary values on mounting of component
  componentDidMount() {
    const { name, price, qty, qtysold, pricesold, dateadded } = this.props;
    this.setState({ name, price, qty, qtysold, pricesold, dateadded });
  }

  //function to trigger form view and close item view
  handleEditClick = () => this.setState({ openEditForm: true });

  //calls parent's function to updated item
  handleUpdate = ({ name, price, qty, qtysold, pricesold, dateadded }) => {
    const updatedItem = {
      id: this.props.id,
      name,
      price,
      qty,
      qtysold,
      pricesold,
      dateadded
    };
    this.props.handleUpdate(updatedItem);
    this.handleCancel();
  };

  //calls parent's function to delete item from store
  handleDelete = () => this.props.handleDelete(this.props.id);

  //function to close form
  handleCancel = () => this.setState({ openEditForm: false });

  render() {
    return (
      <>
        {!this.state.openEditForm ? (
          <div className="menu-row">
            <div className="menu-item-name">{this.state.name}</div>
            <div className="menu-item-price">{this.state.price}</div>
            <div className="menu-item-price">{this.state.qty}</div>
            <div className="menu-item-price">{this.state.qtysold}</div>
            <div className="menu-item-price">{this.state.pricesold}</div>
            <div className="menu-item-price">{this.state.dateadded}</div>
            <div className="operations">
              <span onClick={this.handleEditClick} className="btn edit">
                <i className="fas fa-pen"></i>
              </span>
              <span onClick={this.handleDelete} className="btn delete">
                <i className="fas fa-trash"></i>
              </span>
            </div>
          </div>
        ) : (
          <Form
            name={this.state.name}
            price={this.state.price}
            qty={this.state.qty}
            qtysold={this.state.qtysold}
            pricesold={this.state.pricesold}
            dateadded={this.state.dateadded}
            closeForm={this.handleCancel}
            updateItem={this.handleUpdate}
          />
        )}
      </>
    );
  }
}
